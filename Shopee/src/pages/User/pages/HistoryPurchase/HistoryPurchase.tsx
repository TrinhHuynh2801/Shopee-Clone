...
export default function HistoryPurchase() {
  ...
  const purchaseTabsLink = purchaseTabs.map((tab) => (
    <Link
      ...
      className={`flex flex-1 items-center justify-center border-b-2 bg-white py-4 text-center  ${status === tab.status ? 'border-b-shopee text-shopeeText' : 'border-b-black/10 text-gray-900'}`}
    >
      {tab.name}
    </Link>
  ))
              ...
              <div key={purchase._id} className='mt-4 rounded-sm border-black/10 bg-white p-6 text-gray-800 shadow-sm'>
                ...
                >
                  ...
                  <div className='ml-3 flex-shrink-0'>
                    ...
                    <span className='ml-2 truncate text-shopeeText'>
                      ₫{formatNumberWithPeriods(purchase.product.price)}
                    </span>
                  </div>
                </Link>
                <div className='flex justify-end'>
                  <div>
                    ...
                    <span className='ml-4 text-xl text-shopeeText'>
